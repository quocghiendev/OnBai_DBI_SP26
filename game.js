...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const SHAPES = [
    ...
    { matrix: [[1, 1], [1, 1]], color: 4 },
    // 3x3 Square
    { matrix: [[1, 1, 1], [1, 1, 1], [1, 1, 1]], color: 5 },
    // L-Shapes (Small 2x2)
    { matrix: [[1, 0], [1, 1]], color: 5 },
    { matrix: [[0, 1], [1, 1]], color: 6 },
    { matrix: [[1, 1], [1, 0]], color: 1 },
    { matrix: [[1, 1], [0, 1]], color: 2 },
    // L-Shapes (3x3 Big L)
    { matrix: [[1, 0, 0], [1, 0, 0], [1, 1, 1]], color: 6 },
    { matrix: [[0, 0, 1], [0, 0, 1], [1, 1, 1]], color: 3 },
    // T-Shapes
    { matrix: [[1, 1, 1], [0, 1, 0]], color: 4 },
    { matrix: [[0, 1, 0], [1, 1, 1]], color: 5 },
    { matrix: [[1, 0], [1, 1], [1, 0]], color: 2 },
    // Corner 2x2
    { matrix: [[1, 1], [1, 0]], color: 1 }
  ];
  ...
  function clearCellPreviews() {
    ...
    previewCells.forEach(cell => cell.classList.remove('preview-valid', 'preview-invalid', 'ghost-valid', 'ghost-invalid'));
  }
  ...
  function placeBlock(block, r, c) {
    ...
    block.matrix.forEach(row => row.forEach(val => { if(val === 1) placedCellsCount++; }));
  ...
  function filterReviewQuestions() {
    ...
    filteredQuestionsList = allQuestions.filter(q => {
      ...
      const qMatch = !term || q.question.toLowerCase().includes(term) || q.options.some(opt => opt.text.toLowerCase().includes(term));
